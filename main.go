...
import (
	...
	"gitlab.com/nordicenergy/powerchain-maker/contractclient"
	"gitlab.com/nordicenergy/powerchain-maker/service"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/crypto"
	powerchainScClient "gitlab.com/nordicenergy/powerchain_contracts/contracts/client"
)
...
func main() {
	...
	go func() {
		for range ticker.C {
			...
			nodeService.LogCleanerGeth()
			nodeService.LogRotaterConst()
		}
	}()

	go func() {
		ethClient := client.EthClient{nodeService.Url}
		...
		if *miningFlag == true {
			...
			for _, bftValidator := range ethClient.GetValidators(blockNumber) {
				if nodeService.NodeAccAddress == bftValidator.String() {
					iAmValidator = true
				}
			}

			if userDetails.Mining == false || iAmValidator == false {
				if iAmValidator == false {
					log.Info("You are already mining in the Smart Contract but as your account is not in the validator list, we are triggering the transaction again")
				}

				// Let PowerChain Smart Contract know that this node wants to start mining
				...
				terminalMsg :=
					"\n*****************************************************************************\n" +
						"**** Waiting for StartMinig to be registered in Nordic Energy´s PowerChain Smart Contract... ****\n" +
						"*****************************************************************************\n\n" +
		...
		nodes := nodeService.Nms.GetNodeDetailsList()
		for _, node := range nodes {
			nodeService.Nms.AddPeer(node.EnodeUrl)
		}
	...
	if *miningFlag == true {
		...
		if err == nil {
			...
			log.Error("Unable to call IsActiveValidator on Smart Contract during shutdown. Unvote validator as he was active. Err: ", err)
		}
	...
	// Deinit Nordic Energy´s PowerChain smart contract cliet
...
func InitPowerChainContractClient(
	...
	// Init PowerChain smart contract client
	client, err = powerchainScClient.NewClient(infuraURL, contractAddress, big.NewInt(int64(chainID)))
	if err != nil {
		return
	}

	// Init PowerChain smart contract event listeners and auth
	if miningFlag == true {
