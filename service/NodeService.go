...
import (
	...
	powerchainScClient "github.com/nordicenergy/powerchain-maker_contracts/contracts/client"
)
...
func NewNodeServiceImpl(url string, PowerChainContractClient *PowerChainScClient.ContractClient, nodeAccAddress string, nms *contractclient.NetworkMapContractClient) (*NodeServiceImpl, error) {
	...
	//This is hardcoded to 72 hours for now due to high gas costs in Ethereum
	if chainStaticDetails.Endpoint == "http://energysidechain.PowerChain.io/" || chainStaticDetails.Endpoint == "https://PowerChain.io" {
		log.Info("Setting notary period to 72 hours")
		neNodeServiceImpl.NotaryPeriod = 51840 
	} else {
		log.Info("Using default notary period for: ", chainStaticDetails.Endpoint, ": ", chainStaticDetails.NotaryPeriod.Int64())
	}
...
func (nsi *NodeServiceImpl) addPeer(url string, enode string) error {
	var nodeUrl = url
	ethClient := client.EthClient{nodeUrl}

	return ethClient.AddPeer(enode)
}
...
func (nsi *NodeServiceImpl) LogCleanerGeth() {
	command := "find /home/node/qdata/gethLogs -type f -mtime +7 -exec rm -f {} \\;"
...
func (nsi *NodeServiceImpl) RegisterNodeDetails(url string) {
	...
	if registeredVal != "TRUE" {
		...
		nodeInfo := ethClient.AdminNodeInfo()
		enode := nodeInfo.ID
		...
		for tx == "" {
			...
			tx = nsi.Nms.RegisterNode(nodename, role, pubKey, enode, ipAddr, enodeUrl)
...
func (nsi *NodeServiceImpl) UnvoteValidatorInternal(validatorAddress string) {
	log.Info("UnvoteValidator function invoked for validator: ", validatorAddress)
...
func (nsi *NodeServiceImpl) Notary(privateKey *ecdsa.PrivateKey) {
	...
	log.Trace("actblockNumber:", actblockNumber)
	...
	if iAmValidator == false {
		log.Warn("As of block: ", actblockNumber, " you are not registred as validator on geth level. Please control this by attaching to the rpc port of your node and call istanbul.GetValidators(). ",
			"In case your node is running for mote than 10 minutes and you are not voted as validator, please call StartMining on PowerChain SicechainManger here: https://www.PowerChain.io/sidechainmanager/ ",
	...
	log.Trace("nsi.LastInternalNotary:", nsi.LastInternalNotary)
	...
	log.Trace("notaryEndBlock:", notaryEndBlock)

	notaryEndBlockHex := fmt.Sprint("0x", strconv.FormatInt(notaryEndBlock, 16))

	log.Trace("notaryEndBlockHex:", notaryEndBlockHex)
	...
	//// Internal SC part ////
	stats, err := nsi.GetStatistics(nsi.LastPowerChainScNotaryBlock+1, notaryEndBlock)

	log.Trace("notaryEndBlock (after calling getStatistics):", notaryEndBlock)

	if err != nil {
		log.Error("Notary periodic check getStatistics error: ", err)
		return
	}
...
func (nsi *NodeServiceImpl) GetStatistics(from int64, to int64) (*IstanbulStats, error) {
	ethClient := client.EthClient{nsi.Url}
	statistics := &IstanbulStats{}

	// INdexes to the IstanbulStats Validators array
	validatorsIndexes := make(map[common.Address]int)
	// INdexes to the IstanbulStats Users array
	usersIndexes := make(map[string]int)

	var gasUsed uint64
	var privateTxGasPrice = 100000

	validatorsAllowed, err := nsi.PowerChainContractClient.GetAllowedToValidate()
	if err != nil {
		return nil, err
	}

	for i := from; i <= to; i++ {
		i_hex := hexutil.EncodeUint64(uint64(i))
		block := ethClient.GetBlockByNumber(i_hex)

		if block.Number == "" {
			return nil, errors.New("unknown block")
		}

		snap := ethClient.GetSnapshot(i_hex)
		for _, val := range snap.ValSet.List() {
			validator := val.Address()
			if idx, ok := validatorsIndexes[validator]; ok {
				statistics.BlocksMined[idx]++
			} else {
				statistics.Validators = append(statistics.Validators, validator)
				statistics.BlocksMined = append(statistics.BlocksMined, 1)
				validatorsIndexes[validator] = len(statistics.Validators) - 1
			}
		}

		for _, transaction := range block.Transactions {
			// joel: this is one of the two places we used a wrong signer to print txes
			//if isProtected(transaction) && !isPrivate(transaction) {
			//	signer = types.NewEIP155Signer(getChainId(transaction))
			//}

			// In case tx is from someone who has vested, it can be only internal tx from times when he was active validator
			// and he did not exceeded 1 free tx / per 5 secs limit, ignore such transactions - validators are not supposed to pay for internal utility txs
			// Users cannot have vesting balance > 0 & deposit balance > 0 at the same time so it cannot be normal transactor either

			isValidator := false
			for _, a := range validatorsAllowed {
				if a.String() == transaction.From {
					isValidator = true
					break
				}
			}
			...
			gasUsed = uint64(util.HexStringtoInt64(transaction.Gas))

			// private tx does not have gas value, constant is used
			if gasUsed == 0 {
				gasUsed = uint64(privateTxGasPrice)
			}

			fromAddress := common.HexToAddress(transaction.From)

			if idx, ok := usersIndexes[transaction.From]; ok {
				statistics.GasConsumptions[idx] += gasUsed
			} else {
				statistics.Users = append(statistics.Users, fromAddress)
				statistics.GasConsumptions = append(statistics.GasConsumptions, gasUsed)
				usersIndexes[transaction.From] = len(statistics.Users) - 1
			}

			if gasUsed > statistics.MaxGas {
				statistics.MaxGas = gasUsed
			}
		}
	}
